...
# CONFIG
# =========================
BASE_DIR = "/storage/emulated/0/badapple_xdk60fps"
...
def image_to_ascii(img, term_width, term_height):
    ...
    # keep 16:9 inside terminal size
    max_w = term_width - 4
    max_h = term_height - 6
    aspect_correction = CHAR_ASPECT

    target_w = max_w
    target_h = int((h / w) * target_w * aspect_correction)
    if target_h > max_h:
        target_h = max_h
        target_w = int((w / h) * target_h / aspect_correction)

    img = img.resize((target_w, target_h))

    pixels = img.getdata()
    chars = [map_pixel_to_char(p) for p in pixels]
    ascii_lines = ["".join(chars[i:i+target_w]) for i in range(0, len(chars), target_w)]
...
def download_video(url, outpath):
    ...
    ydl_opts = {"format":"bestvideo[height<=1080][fps<=60]+bestaudio/best",
                "outtmpl":outpath,"merge_output_format":"mp4","quiet":True}
...
def get_fps_and_duration(video_path, url):
    with yt_dlp.YoutubeDL({"quiet":True}) as ydl:
        ...
        return info.get("fps",60), info.get("duration",0)
...
def save_ascii_frames(frame_files, term_width, term_height):
    ...
    for i,fpath in enumerate(frame_files):
        ...
        print(f"\rASCII {i+1}/{len(frame_files)}", end="", flush=True)
    print("\nConversion to ASCII completed!")
...
def main():
    if len(sys.argv)<2:
        print("Usage: python badapple.py <URL>")
        sys.exit(1)
    url=sys.argv[1]
    ensure_dirs()
